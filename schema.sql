...
CREATE TABLE Performs (
    ArtistID VARCHAR(50) REFERENCES Artists(ArtistID) ON DELETE CASCADE,
    SongID VARCHAR(50) REFERENCES Songs(SongID) ON DELETE CASCADE,
...
CREATE TABLE IsGenre (
    ArtistID VARCHAR(50) REFERENCES Artists(ArtistID) ON DELETE CASCADE,
    GenreID VARCHAR(50) REFERENCES Genres(GenreID) ON DELETE CASCADE,
    PRIMARY KEY (ArtistID, GenreID)
);

CREATE TABLE InAlbum (
    SongID VARCHAR(50) REFERENCES Songs(SongID) ON DELETE CASCADE,
    AlbumID VARCHAR(50) REFERENCES Albums(AlbumID) ON DELETE CASCADE,
...
CREATE TABLE CreatesPlaylist (
    UserID VARCHAR(50) REFERENCES "Users"(UserID) ON DELETE CASCADE,
    PlaylistID VARCHAR(50) REFERENCES Playlists(PlaylistID) ON DELETE CASCADE,
...
CREATE TABLE FollowsUser (
    FollowerID VARCHAR(50) REFERENCES "Users"(UserID) ON DELETE CASCADE,
    FollowedID VARCHAR(50) REFERENCES "Users"(UserID) ON DELETE CASCADE,
